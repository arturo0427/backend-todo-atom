...
export class FirestoreTaskRepository implements TaskRepository {
  ...
  async listByUser(userId: string): Promise<Task[]> {
    const snap = await this.collection.where('userId', '==', userId).orderBy('createdAt', 'desc').get();
  ...
  async update(taskId: string, patch: Partial<Omit<Task, 'id'|'userId'|'createdAt'>>): Promise<Task> {
  ...
  async delete(taskId: string): Promise<void> { await this.collection.doc(taskId).delete(); }
}
